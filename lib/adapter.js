...
export default class Adapter {
  ...
  restoreCurrentOrder() {
    return STORAGE.getItem('currentOrder').then(serializedOrder => {
  ...
  persistCurrentOrder(order) {
    ...
    if(order.creditCard) {
      ...
      return STORAGE.setItem('currentOrder', CircularJSON.stringify(sanitizeCreditCard(order))).then(() => {
        order.creditCard = creditCardData;
        return order;
      });
    }
    return STORAGE.setItem('currentOrder', CircularJSON.stringify(order)).then(() => order);
  }
  ...
  restoreCustomerToken() {
    return STORAGE.getItem('customerToken').then(customerToken => {
      this.customerToken = customerToken;
    });
  }

  persistCustomerToken(customerToken) {
    return STORAGE.setItem('customerToken', customerToken).then(token => {
